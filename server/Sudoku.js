...
class Sudoku {
  constructor(req,sudoku) {
    if (!sudoku) {
      this.sudoku = createPuzzle(req.body.coef);
    } else {
      this.sudoku = sudoku;
    }
    this.solvedSudoku = [];
  ...
  validate(req , res, next) {
    const check = isValidSolution(this.sudoku);
    if(check){
      gameController.addGame(req, res, true, next);
      userController.updateRating(req, res, next);
    }else{
      gameController.addGame(req, res, false, next)
    }
    return check;
  }
...
function isValidSolution(grid) {
  ...
  const check = SudokuUtil.isValidPuzzle(grid);
  return check;
}
...
function createPuzzle(coef) {
  let puzzle = getRandomSudoku();
  let solution = solve(puzzle);
  if (solution) {
    for (let i = 0; i < 9; i++) {
      for (let j = 0; j < 9; j++) {
        if (Math.random() > coef) puzzle[i][j] = 0;
      }
    }
  }
  return puzzle;
